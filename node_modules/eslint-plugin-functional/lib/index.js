...
function createRuleUsingFunction(name, meta, defaultOptions, createFunction) {
    return utils.ESLintUtils.RuleCreator((ruleName) => `https://github.com/eslint-functional/eslint-plugin-functional/blob/v4.4.1/docs/rules/${ruleName}.md`)({
        name,
        meta,
        defaultOptions,
...
function getTypeOfNode(node, contextOrServices) {
...
function getESTreeNode(node, contextOrServices) {
    const parserServices = isParserServices(contextOrServices)
        ? contextOrServices
        : getParserServices(contextOrServices);
    if (parserServices === null) {
...
function isParserServices(contextOrServices) {
...
const schema$f = [
    {
        type: "object",
        properties: deepmergeTs.deepmerge(ignorePatternOptionSchema, ignorePrefixSelectorOptionSchema, {
            allowRestParameter: {
                type: "boolean",
            },
...
const rule$f = createRuleUsingFunction(name$f, meta$f, defaultOptions$f, (context, options) => {
    const [optionsObject] = options;
    ...
    return {
        ...Object.fromEntries(fullFunctionSelectors.map((selector) => [selector, checkFunction$2])),
...
function buildFixer(context, node, caller) {
    return (fixer) => {
